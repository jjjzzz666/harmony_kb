...
void SampleBitMap::DrawTextNAbility()
{
    ...
    for (int idx = 0; idx < count; idx++) {
        ...
        runBuffer->pos[idx * two] = posX[idx];
        runBuffer->pos[idx * two + one] = posY[idx];
    }
    // 通过文本构造器创建文本
    ...
    OH_Drawing_TextEncoding type = OH_Drawing_TextEncoding::TEXT_ENCODING_UTF16;
    OH_Drawing_TextBlob* textBlob2 = OH_Drawing_TextBlobCreateFromText(buffer, size * sizeof(char16_t), font, type);
    // 创建一个画刷Brush对象，Brush对象用于形状的填充
    cBrush_ = OH_Drawing_BrushCreate();
