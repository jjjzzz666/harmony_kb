...
public:
    ...
    int32_t SetId(const std::string &id)
    {
        ArkUI_AttributeItem item = {.string = id.c_str()};
        auto err = nativeModule_->setAttribute(handle_, NODE_ID, &item);
        return err;
    }

    int32_t SetMargin(float margin)
    {
...
protected:
    ...
    void OnRemoveAll() override { nativeModule_->removeAllChildren(handle_); }
};
} // namespace NativeModule
