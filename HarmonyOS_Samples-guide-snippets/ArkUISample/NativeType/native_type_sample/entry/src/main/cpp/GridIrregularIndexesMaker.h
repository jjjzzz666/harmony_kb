...
#ifndef SCROLLABLENDK_GRID_IRREGULAR_INDEXES_NODE_H
#define SCROLLABLENDK_GRID_IRREGULAR_INDEXES_NODE_H
...
class GridIrregularIndexesMaker : public BaseNode {
public:
    struct GridParam {};
    static ArkUI_NodeHandle CreateNativeNode();

    GridIrregularIndexesMaker()
          ...
          nodeApi_(NodeApiInstance::GetInstance()->GetNativeNodeAPI())
    {
        ...
        layoutOptions1_ = std::make_shared<GridLayoutOptions>();
        layoutOptions2_ = std::make_shared<GridLayoutOptions>();
    }

    ~GridIrregularIndexesMaker() override
    {
        scrollEventGuard_.Release();
        nodeAdapter_.reset();
        layoutOptions1_ = nullptr;
        layoutOptions2_ = nullptr;
    }
    ...
    {
        SetAttributeFloat32(nodeApi_, GetHandle(), NODE_SCROLL_BAR_DISPLAY_MODE, displayMode);
    }
    // ========================================
    // 适配器设置
    // ========================================
...
private:
    static std::shared_ptr<GridIrregularIndexesMaker> BuildGrid1();
    static std::shared_ptr<GridIrregularIndexesMaker> BuildGrid2();
    ArkUI_NativeNodeAPI_1* nodeApi_ = nullptr;
    std::shared_ptr<ArkUINodeAdapter> nodeAdapter_;
    ScrollEventGuard scrollEventGuard_;
    static std::shared_ptr<GridLayoutOptions> layoutOptions1_;
    static std::shared_ptr<GridLayoutOptions> layoutOptions2_;
};

#endif // SCROLLABLENDK_GRID_IRREGULAR_INDEXES_NODE_H
