...
# NDKMinResolutionSample

### 介绍

本示例主要展示了相机的相关功能，使用[libohcamera.so]
接口实现相机的预览、拍照、录像、前后置摄像头切换进行拍照、录像、设置不同分辨率进行拍照、录像，以及闪光灯、变焦、对焦、曝光、参考线以及动态调整帧率等控制类功能。
...
10. 进入预览界面，预览正常，进入拍照模式，点击拍照按钮，拍照正常，点击左下角相册图标，能够跳转到图库，图片保存正常，打开图片显示正常，且显示为HDR照片。
11. 进入预览界面，预览正常，切换到录像模式，点击录像，开始录像，再点击停止录像按钮，录像成功，点击左下角相册图标，能够跳转到图库，录像文件保存正常，播放录像文件正常。
12. 进入预览界面，预览正常，切换到前置摄像头，点击拍照按钮，拍照正常，点击左下角相册图标，能够跳转到图库，图片保存正常，且显示为HDR照片。
13. 进入预览界面，预览正常，切换到前置摄像头，切换到录像模式，点击录像，开始录像，再点击停止录像按钮，录像成功，点击左下角相册图标，能够跳转到图库，录像文件保存正常，播放录像文件正常。
14. 进入预览界面，预览正常，点击照片分辨率和视频分辨率可以设置不同的分辨率进行拍照和录像。
...
* 相机功能接口实现在CameraManager.cpp中，源码参考：[CameraManager.cpp](entry/src/main/cpp/camera_manager.cpp)
    ...
    * 设置分辨率相关接口封装在SetPreviewResolution、SetPhotoResolution、SetVideoResolution接口中，对创建PreviewOutput、PhotoOutput以及VideoOutput时使用的Profile进行修改，实现设置不同分辨率进行拍照和录像的功能
...
* 相机预览、拍照、录像功能、前后置切换功能实现调用侧位于Index.ets，modeSwitchPage.ets，ResolutionService.ets，main.cpp中，源码参考：[Index.ets](entry/src/main/ets/pages/Index.ets)，[ModeSwitchPage.ets](entry/src/main/ets/views/ModeSwitchPage.ets)，[ResolutionService.ets](entry/src/main/ets/common/ResolutionService.ets)，[main.cpp](entry/src/main/cpp/main.cpp)
    ...
    * 设置不同分辨率进行拍照和录像：设置分辨率接口位于ResolutionService.ets，通过选择不同照片分辨率以及视频分辨率，调用cameraDemo.setPreviewResolution、cameraDemo.setPhotoResolution、cameraDemo.setVideoResolution,实际是调用main.cpp下的SetPreviewResolution、SetPhotoResolution、SetVideoResolution接口，最终调用到CameraManager.cpp下的SetPreviewResolution、SetPhotoResolution、SetVideoResolution接口来实现不同分辨率的拍照和录像。


* 相机闪光灯、变焦、对焦、曝光功能实现调用侧位于FlashingLightPage.ets，SlidePage.ets，FocusAreaPage.ets，VideoRatePage.ets中，源码参考：[FlashingLightPage.ets](entry/src/main/ets/views/FlashingLightPage.ets)，[SlidePage.ets](entry/src/main/ets/views/SlidePage.ets)，[FocusAreaPage.ets](entry/src/main/ets/views/FocusAreaPage.ets)，[VideoRatePage.ets](entry/src/main/ets/views/VideoRatePage.ets)，[main.cpp](entry/src/main/cpp/main.cpp)
