...
* 相机功能接口实现在CameraManager.cpp中，源码参考：[CameraManager.cpp](entry/src/main/cpp/CameraManager.cpp)
    ...
    * 拍照功能相关接口封装在StartPhoto接口中，主要包含以下流程：调用SessionStop关闭session，调用SessionBegin做session的一个预置动作，调用CreatePhotoOutputWithoutSurfaceId创建相机输出，调用OH_CaptureSession_AddPhotoOutput将photoOutput添加至session中，调用PhotoOutputRegisterPhotoAssetAvailableCallback注册分段式拍照回调，调用SessionCommitConfig提交session，在调用SessionStart开启session，最后调用TakePicture接口开启拍照动作。
...
* 相机预览、分段式拍照功能、前后置切换、动态照片功能实现调用侧位于Index.ets，modeSwitchPage.ets，main.cpp中，源码参考：[Index.ets](entry/src/main/cpp/ets/pages/Index.ets)，[modeSwitchPage.ets](entry/src/main/cpp/ets/views/modeSwitchPage.ets)，[main.cpp](entry/src/main/cpp/main.cpp)
    ...
    * 拍照：开启拍照位于ModeSwitchPage.ets下的cameraDemo.startPhotoOrVideo接口，将modelBagCol的值，videoId，拍照的surfaceID或者录像的surfaceId传入接口startPhotoOrVideo。跳转到main.cpp中的StartPhotoOrVideo接口，将传下来的参数进行格式转换，再调用CameraManager对象下的StartPhotoWithOutSurfaceId接口开启拍照操作。
