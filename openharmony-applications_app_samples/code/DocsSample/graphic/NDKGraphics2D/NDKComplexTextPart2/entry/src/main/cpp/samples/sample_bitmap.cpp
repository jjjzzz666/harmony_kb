...
void SampleBitMap::DrawMultilineText()
{
    // 创建 FontCollection，FontCollection 用于管理字体匹配逻辑
    OH_Drawing_FontCollection *fc = OH_Drawing_CreateSharedFontCollection();
    ...
    // 设置页面最大宽度
    double maxWidth = width_;
    OH_Drawing_TypographyLayout(typographyBreakAll, maxWidth);
    // 将文本绘制到画布上
    OH_Drawing_TypographyPaint(typographyBreakAll, cCanvas_, 0, 100);
...
void SampleBitMap::AddFontFeature()
{
    // 创建一个 TypographyStyle，创建 TypographyCreate 时需要使用
    OH_Drawing_TypographyStyle *typoStyle = OH_Drawing_CreateTypographyStyle();
    // 设置文本对齐方式为居中
...
napi_value SampleBitMap::NapiDrawMultilingualText(napi_env env, napi_callback_info info)
{
    DRAWING_LOGI("NapiDrawMultilingualText");
    if ((env == nullptr) || (info == nullptr)) {
        DRAWING_LOGE("NapiDrawMultilingualText: env or info is null");
        return nullptr;
    }

    napi_value thisArg;
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilingualText: napi_get_cb_info fail");
        return nullptr;
    }

    napi_value exportInstance;
    if (napi_get_named_property(env, thisArg, OH_NATIVE_XCOMPONENT_OBJ, &exportInstance) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilingualText: napi_get_named_property fail");
        return nullptr;
    }

    OH_NativeXComponent *nativeXComponent = nullptr;
    if (napi_unwrap(env, exportInstance, reinterpret_cast<void **>(&nativeXComponent)) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilingualText: napi_unwrap fail");
        return nullptr;
    }
    ...
    if (OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize) != OH_NATIVEXCOMPONENT_RESULT_SUCCESS) {
        DRAWING_LOGE("NapiDrawMultilingualText: Unable to get XComponent id");
        return nullptr;
    }
    ...
    if (render != nullptr) {
        render->Prepare();
        render->Create();
        render->DrawMultilingualText();
        render->DisPlay();
        render->Destroy();
        DRAWING_LOGI("DrawMultilingualText executed");
    } else {
...
napi_value SampleBitMap::NapiDrawMultilineText(napi_env env, napi_callback_info info)
{
    DRAWING_LOGI("NapiDrawMultilineText");
    if ((env == nullptr) || (info == nullptr)) {
        DRAWING_LOGE("NapiDrawMultilineText: env or info is null");
        return nullptr;
    }

    napi_value thisArg;
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilineText: napi_get_cb_info fail");
        return nullptr;
    }

    napi_value exportInstance;
    if (napi_get_named_property(env, thisArg, OH_NATIVE_XCOMPONENT_OBJ, &exportInstance) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilineText: napi_get_named_property fail");
        return nullptr;
    }

    OH_NativeXComponent *nativeXComponent = nullptr;
    if (napi_unwrap(env, exportInstance, reinterpret_cast<void **>(&nativeXComponent)) != napi_ok) {
        DRAWING_LOGE("NapiDrawMultilineText: napi_unwrap fail");
        return nullptr;
    }
    ...
    uint64_t idSize = OH_XCOMPONENT_ID_LEN_MAX + 1;
    if (OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize) != OH_NATIVEXCOMPONENT_RESULT_SUCCESS) {
        DRAWING_LOGE("NapiDrawMultilineText: Unable to get XComponent id");
        return nullptr;
    }
    DRAWING_LOGI("ID = %{public}s", idStr);
    std::string id(idStr);
    ...
    if (render != nullptr) {
        render->Prepare();
        render->Create();
        render->DrawMultilineText();
        render->DisPlay();
        render->Destroy();
        DRAWING_LOGI("DrawMultilineText executed");
    } else {
        DRAWING_LOGE("render is nullptr");
    }
    return nullptr;
...
napi_value SampleBitMap::NapiAddFontFeature(napi_env env, napi_callback_info info)
{
    DRAWING_LOGI("NapiAddFontFeature");
    if ((env == nullptr) || (info == nullptr)) {
        DRAWING_LOGE("NapiAddFontFeature: env or info is null");
        return nullptr;
    }

    napi_value thisArg;
    if (napi_get_cb_info(env, info, nullptr, nullptr, &thisArg, nullptr) != napi_ok) {
        DRAWING_LOGE("NapiAddFontFeature: napi_get_cb_info fail");
        return nullptr;
    }

    napi_value exportInstance;
    if (napi_get_named_property(env, thisArg, OH_NATIVE_XCOMPONENT_OBJ, &exportInstance) != napi_ok) {
        DRAWING_LOGE("NapiAddFontFeature: napi_get_named_property fail");
        return nullptr;
    }

    OH_NativeXComponent *nativeXComponent = nullptr;
    if (napi_unwrap(env, exportInstance, reinterpret_cast<void **>(&nativeXComponent)) != napi_ok) {
        DRAWING_LOGE("NapiAddFontFeature: napi_unwrap fail");
        return nullptr;
    }
    ...
    uint64_t idSize = OH_XCOMPONENT_ID_LEN_MAX + 1;
    if (OH_NativeXComponent_GetXComponentId(nativeXComponent, idStr, &idSize) != OH_NATIVEXCOMPONENT_RESULT_SUCCESS) {
        DRAWING_LOGE("NapiAddFontFeature: Unable to get XComponent id");
        return nullptr;
    }
    DRAWING_LOGI("ID = %{public}s", idStr);
    std::string id(idStr);
    ...
    if (render != nullptr) {
        render->Prepare();
        render->Create();
        render->AddFontFeature();
        render->DisPlay();
        render->Destroy();
        DRAWING_LOGI("AddFontFeature executed");
    } else {
        DRAWING_LOGE("render is nullptr");
    }
    return nullptr;
...
void SampleBitMap::Export(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor desc[] = {
        {"drawMultilineText", nullptr, SampleBitMap::NapiDrawMultilineText, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"addFontFeature", nullptr, SampleBitMap::NapiAddFontFeature, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"drawMultilingualText", nullptr, SampleBitMap::NapiDrawMultilingualText, nullptr, nullptr, nullptr, napi_default, nullptr}};
